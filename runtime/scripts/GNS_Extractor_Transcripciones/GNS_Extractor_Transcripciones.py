...
def obtener_transcript_url(config: Config, token: str, conversation_id: str, communication_id: str, logger: logging.Logger) -> Optional[str]:
    try:
        return get_transcript_signed_url(config, token, conversation_id, communication_id, logger)
...
def buscar_transcript_url(config: Config, token: str, conversation_details: Dict[str, Any], logger: logging.Logger) -> Dict[str, Any]:
    ...
    if not conversation_id:
        return {
            "estado": "SIN_CONVERSATION_ID",
            "error": "La conversacion no contiene conversationId.",
            "intentos": intentos,
        }
    ...
    for candidato in candidatos:
        ...
        try:
            ...
            resultado = "OK" if transcript_url else "404/SIN_URL"
            intentos.append({
                "communication_id": communication_id,
                "purpose": purpose,
                "recording": recording,
            ...
            if transcript_url:
                return {
                    "conversation_id": conversation_id,
                    "communication_id_correcto": communication_id,
                    "purpose": purpose,
                    "recording": recording,
    ...
    return {
        "conversation_id": conversation_id,
        "estado": "SIN_TRANSCRIPCION_API",
        "error": "Ningun sessionId candidato devolvio transcripturl.",
        "intentos": intentos,
    }
...
def get_output_columns(rows: List[Dict[str, Any]]) -> List[str]:
    all_cols: List[str] = []
    for row in rows:
        for key in row.keys():
...
def write_csv(rows: List[Dict[str, Any]], output_path: str, logger: logging.Logger) -> None:
    output_path = resolve_output_path(output_path, "csv")
    out_dir = os.path.dirname(output_path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    all_cols = get_output_columns(rows)
...
def write_excel(rows: List[Dict[str, Any]], output_path: str, logger: logging.Logger) -> None:
    output_path = resolve_output_path(output_path, "xlsx")
    out_dir = os.path.dirname(output_path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    ...
    try:
        ...
        raise RuntimeError("Para generar Excel se requiere openpyxl instalado.") from exc

    columns = get_output_columns(rows)
    wb = Workbook()
    ws = wb.active
    ws.title = "Transcripciones"
    ...
    for col_index, column in enumerate(columns, start=1):
        ...
        ws.column_dimensions[get_column_letter(col_index)].width = min(max_len + 2, 70)

    wb.save(output_path)
    logger.info("Excel generado: %s", output_path)


def write_output(rows: List[Dict[str, Any]], output_path: str, output_format: str, logger: logging.Logger) -> None:
...
def process_conversation_transcripts(
    conv: Dict[str, Any],
    idx: int,
    total: int,
    config: Config,
    ...
    if config.output_mode == "transcript_campania":
        try:
            ...
            logger.warning("No se pudo enriquecer contacto/campana para %s: %s", conversation_id, exc)

    base_row = {
        "conversationId": conversation_id,
        "conversationStart_local": format_local_datetime(start_utc, config.timezone_name),
        "conversationEnd_local": format_local_datetime(end_utc, config.timezone_name),
        "conversationDate_local": format_local_date(start_utc, config.timezone_name),
        "conversationStart": format_local_datetime(start_utc, config.timezone_name),
        "conversationEnd": format_local_datetime(end_utc, config.timezone_name),
        "conversationStart_utc": start_utc,
        "conversationEnd_utc": end_utc,
        ...
        "wrapUpCodeId_ultima": last_semicolon_value(";".join(wrapup_ids)),
        "conclusion_original": wrapup_names,
        "conclusion_ultima": last_semicolon_value(wrapup_names),
    ...
    base_row.update(contact_flat)

    transcript_lookup = buscar_transcript_url(config, token, conv, logger)
    intentos_json = json.dumps(transcript_lookup.get("intentos", []), ensure_ascii=False)

    if transcript_lookup.get("estado") != "OK":
        transcript_not_found_count += 1
        row = {
            **base_row,
            "communicationId": "",
            "communication_id_usado": "",
            "communication_purpose_usado": "",
            "transcript_estado": transcript_lookup.get("estado", "SIN_TRANSCRIPCION_API"),
            "transcript_error": transcript_lookup.get("error", ""),
            "transcript_intentos": intentos_json,
            "phrases_count": 0,
            "transcript_first_marker": "",
    ...
    try:
        communication_id = transcript_lookup.get("communication_id_correcto", "")
        signed_url = transcript_lookup["transcript_url"]
        ...
        row = {
            **base_row,
            ...
            "communication_id_usado": communication_id,
            "communication_purpose_usado": transcript_lookup.get("purpose", ""),
            "transcript_estado": "OK",
            "transcript_error": "",
            "transcript_intentos": intentos_json,
        ...
        errors.append(f"Error descargando transcript conversationId={conversation_id}: {exc}")
...
def main() -> int:
    ...
    try:
        ...
        queue_catalog = get_queue_catalog(config, token, logger)
        ...
        workers = min(config.max_transcript_workers, max(1, total_conversations))
        logger.info("Descargando transcripciones con %s trabajador(es) paralelo(s).", workers)

        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(
                    process_conversation_transcripts,
                    conv,
                    idx,
                    total_conversations,
                    config,
            ...
            for future in as_completed(futures):
                completed += 1
                try:
                    chunk_rows, chunk_not_found, chunk_errors = future.result()
                    rows.extend(chunk_rows)
                    transcript_not_found_count += chunk_not_found
                ...
                if completed == 1 or completed % config.log_every_n == 0 or completed == total_conversations:
                    ...
                    logger.info(
                        "Avance transcripciones: %s/%s | filas=%s | sin transcript=%s | errores=%s",
                        completed,
                        total_conversations,
                        len(rows),
                        transcript_not_found_count,
                        len(errors),
                    )
        ...
        if config.dry_run:
            logger.warning("DRY_RUN=true. No se generara archivo de salida.")
        else:
            write_output(rows, config.output_csv, config.output_format, logger)
