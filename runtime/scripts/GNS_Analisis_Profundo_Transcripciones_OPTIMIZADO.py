...
#                     Ejemplo: C:\PyFlow\runtime\exports\GNS_Transcripciones_20260623_091817.csv
#   OUTPUT_DIR      : Carpeta donde se guardará el Excel final.
#                     Ejemplo: C:\PyFlow\runtime\exports
#   OUTPUT_FILENAME : Nombre opcional del archivo final.
#                     Ejemplo: Analisis_Profundo_Transcripciones_20260623.xlsx
# =========================================================
...
# IMPORTANTE:
# Estructura compatible con PyFlow Manager, tomando como referencia el extractor Genesys.
PYFLOW_PARAMS = {
...
# =========================================================
# Diferencia clave vs. script anterior:
#   - Ya no clasifica por la primera coincidencia.
#   - Ahora calcula puntajes por categoría.
#   - Usa prioridad de motivo raíz para evitar que 'Migración Atlántida HN'
#     absorba llamadas donde el motivo real es OTP, correo, datos, bloqueo, etc.
...
REGLAS = [
    {
        ...
        "patrones_fuertes": [
            ...
            r"no me llega el codigo",
            r"no recibo el codigo",
            r"no me llega el sms",
            r"no recibo sms",
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"correo incorrecto",
            r"correo malo",
            r"correo electronico.*incorrecto",
            r"no me llega el correo",
            r"no recibo el correo",
            r"enlace.*correo.*no"
        ],
        "patrones_medios": [
            r"\bcorreo\b",
...

    {
        ...
        "submotivo": "Usuario no reconocido o inválido",
        "prioridad": 94,
        "confianza_base": "Alta",
        "patrones_fuertes": [r"usuario invalido",r"usuario no reconocido",r"usuario no existe"],
        "patrones_medios": [r"usuario.*inval"]
    },
    {
        "motivo": "Correo inválido",
        "submotivo": "Correo no reconocido o inválido",
        "prioridad": 93,
        "confianza_base": "Alta",
        "patrones_fuertes": [r"correo es invalido",r"correo invalido",r"email invalido"],
        "patrones_medios": [r"correo.*inval"]
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"perdi.*(numero|telefono|correo|celular)",
            r"otro numero",
            r"otro correo",
            r"validar identidad",
            r"validacion de identidad"
        ],
        "patrones_medios": [
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"solicita desbloqueo",
            r"necesito desbloquear"
        ],
        "patrones_medios": [
            r"bloquead",
            r"desbloque"
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"contraseña temporal",
            r"temporal.*(vencid|expiro|expir|caduc)",
            r"enlace.*(vencid|expiro|expir|caduc)",
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"desinstale",
            r"formatee",
            r"perdi.*(celular|telefono|equipo)"
        ],
        "patrones_medios": [
            r"dispositivo",
            r"celular nuevo",
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"web.*(error|falla|no funciona)",
            r"no me deja entrar",
            r"no permite avanzar",
            r"rechaza.*credencial",
            r"credenciales.*incorrect",
    ...
    },
    {
        ...
        "patrones_fuertes": [
            ...
            r"app nueva",
            r"antes ingresaba",
            r"ya no puedo entrar.*(nueva|app|atlantida)"
        ],
        "patrones_medios": [
            r"actualizar la app",
            r"banca digital",
...
def clasificar_motivo(transcripcion: Any) -> Dict[str, str]:
    ...
    for regla in REGLAS:
        ...
        # Puntaje:
        # - Coincidencias fuertes pesan más.
        # - Coincidencias medias ayudan, pero no deben dominar.
        # - La prioridad define desempate de motivo raíz.
        puntaje = (len(fuertes) * 10) + (len(medios) * 3) + regla["prioridad"]

        # Ajuste para no sobreclasificar Migración si solo aparece como contexto.
        if regla["motivo"] == "Migración a Atlántida HN / nueva app":
            if not fuertes:
                puntaje -= 20

        # Ajuste para Consulta general: solo gana si no hay motivo raíz más específico.
    ...
    # Si hay varias categorías con puntajes muy cercanos, marcamos confianza Media.
...
def preparar_base_clasificada(df: pd.DataFrame, col_transcripcion: str) -> pd.DataFrame:
    ...
    base.insert(4, "Puntaje clasificación", clasificaciones["Puntaje clasificación"])
...
def aplicar_formato_excel(output_path: str) -> None:
    ...
    for ws in wb.worksheets:
        ...
        for col_idx in range(1, ws.max_column + 1):
            ...
            if "transcripcion" in header_norm or header_norm == "text":
                width = 90
            elif "evidencia" in header_norm:
                width = 75
            elif "submotivo" in header_norm:
                width = 70
            elif "motivo" in header_norm:
                width = 45
            else:
