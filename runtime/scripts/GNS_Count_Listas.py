...
def generar_excel_con_resumen(df: pd.DataFrame, output_file: Path) -> None:
    ...
    resumen = df.pivot_table(
        ...
        aggfunc={"id": "count", "contactsCount": "sum"},
        fill_value=0,
        dropna=False,
    )
    ...
    total_por_division = df.groupby("division", dropna=False).agg(
        **{"Total listas": ("id", "count"), "Total registros": ("contactsCount", "sum")}
    ).reset_index()
    ...
    limites = pd.DataFrame({
        ...
        "Registros": [limite_registros, total_registros, f"{(total_registros / limite_registros):.0%}"],
    })
...
def main() -> int:
    ...
    try:
        ...
        if include_counts:
            log("Consultando cantidad de contactos por lista...")
            sleep_seconds = env_float("API_SLEEP_SECONDS", 0.2)

            for index, item in enumerate(lists, start=1):
                name = item.get("name") or item.get("id")
                try:
                    log(f"[{index}/{total_lists}] Procesando contactos: {name}")
                    item["contactsCount"] = get_contacts_count(token, item["id"])
                except Exception as exc:
                    count_errors += 1
                    item["contactsCount"] = 0
                    item["contactsCountError"] = str(exc)
                    log(f"[WARN] No se pudo obtener cantidad para {name}: {exc}")
                time.sleep(sleep_seconds)
        else:
            log("INCLUDE_CONTACT_COUNTS=false. No se consultará cantidad de contactos por lista.")
            for item in lists:
                item["contactsCount"] = 0
