...
Para esta troncal específica se usa la regla operativa más confiable:
...
2) Prefiltra por dirección inbound cuando aplica.
3) Filtra localmente por prefijo DNIS/Session DNIS 2275 para evitar falsos positivos del modo EDGE.
4) Agrupa por División usando la División de la cola, usuario, flujo o campaña encontrada en la conversación.
...
PYFLOW_PARAMS = {
    ...
    "TRUNK_DNIS_PREFIXES": {"type": "text", "label": "Prefijos DNIS/Session DNIS de la troncal", "required": False, "default": "2275", "description": "Prefijos separados por coma. Para Tigo_2 el prefijo confirmado es 2275."},
    "TRUNK_NUMBERS": {"type": "text", "label": "DNIS/ANI exactos opcionales", "required": False, "default": "", "description": "Números exactos separados por coma. Opcional; el filtro principal de esta versión es TRUNK_DNIS_PREFIXES=2275."},
    "CALL_DIRECTION": {"type": "select", "label": "Dirección de llamada", "required": True, "options": ["INBOUND", "OUTBOUND", "TODAS"], "default": "INBOUND", "description": "Para el prefijo 2275 se recomienda INBOUND, porque el 2275 corresponde al DNIS de entrada."},
    "OUTPUT_DIR": {"type": "text", "label": "Carpeta de salida del Excel", "required": False, "default": "exports"},
    "EXPORT_DETAIL": {"type": "select", "label": "Exportar detalle", "required": True, "options": ["NO", "SI"], "default": "SI"},
    "FILTER_MODE": {"type": "select", "label": "Modo de filtro", "required": True, "options": ["AUTO", "DNIS_PREFIX", "DNIS", "EDGE", "TODO"], "default": "AUTO", "description": "AUTO usa primero TRUNK_DNIS_PREFIXES, luego DNIS/ANI exacto. EDGE queda disponible solo para validaciones."},
    "CHUNK_DAYS": {"type": "number", "label": "Días por bloque para crear Jobs", "required": False, "default": "7"},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--CALL_DIRECTION", default=env_or_default("CALL_DIRECTION", "INBOUND"))
    parser.add_argument("--OUTPUT_DIR", default=env_or_default("OUTPUT_DIR", "exports"))
    parser.add_argument("--EXPORT_DETAIL", default=env_or_default("EXPORT_DETAIL", "SI"))
...
def segment_duration_seconds(segment: Dict[str, Any]) -> int:
    """Devuelve la duración de un segmento Genesys en segundos.

    Usa segmentStart/segmentEnd cuando están disponibles. Si faltan o son inválidos,
    devuelve 0 para evitar inflar métricas.
    """
...
def extract_time_metrics(conversation: Dict[str, Any]) -> Dict[str, int]:
    """Calcula varias métricas de tiempo para evitar confundir duración total con tiempo atendido.

    Importante:
    - tiempo_conversacion_segundos: conversationEnd - conversationStart. Es la duración completa de la conversación.
    - tiempo_cliente_linea_segundos: rango entre el primer y último segmento del participante customer.
      Es una aproximación al tiempo que el cliente estuvo dentro de la llamada/conversación.
    - tiempo_agente_interact_segundos: suma de segmentos interact del agente. Es la mejor aproximación
      al tiempo hablado/atendido por agentes dentro del detalle de conversación.
    - tiempo_cola_segundos: segmentos delay asociados a ACD/cola.
    - tiempo_ivr_flujo_segundos: segmentos asociados a IVR/flow/bot.
    - tiempo_hold_segundos: segmentos hold.
    - tiempo_acw_segundos: segmentos wrapup del agente.

    No se suma el total de todos los segmentos porque varios participantes pueden existir al mismo tiempo
    y eso duplicaría el tiempo real de una misma llamada.
    """
    metrics = {
    ...
    customer_min: Optional[datetime] = None
    customer_max: Optional[datetime] = None
    ...
    for participant in conversation.get("participants", []) or []:
        ...
        for session in participant.get("sessions", []) or []:
            ...
            for segment in session.get("segments", []) or []:
                ...
                dur = segment_duration_seconds(segment)
                if dur <= 0:
                    continue

                seg_start = parse_genesys_datetime(segment.get("segmentStart"))
                seg_end = parse_genesys_datetime(segment.get("segmentEnd"))

                if purpose == "customer":
                    if seg_start and (customer_min is None or seg_start < customer_min):
                        customer_min = seg_start
                    if seg_end and (customer_max is None or seg_end > customer_max):
                        customer_max = seg_end
                ...
                # En Genesys, el tiempo de cola suele aparecer como delay en ACD o con queueId.
                if seg_type == "delay" and (purpose == "acd" or segment.get("queueId") or session.get("queueId")):
                    metrics["tiempo_cola_segundos"] += dur

                # IVR/flow puede venir como participant purpose o como flowId en session.
                if purpose in {"ivr", "flow", "bot"} or has_flow:
                    # Evitamos contar wrapup/hold de agente por error si hubiera flowId en una sesión rara.
                    if purpose != "agent":
                        metrics["tiempo_ivr_flujo_segundos"] += dur

    if customer_min and customer_max and customer_max > customer_min:
        metrics["tiempo_cliente_linea_segundos"] = int((customer_max - customer_min).total_seconds())

    return metrics


def add_time_formats(row: Dict[str, Any], base_name: str, seconds_value: Any) -> None:
    """Agrega columnas segundos/minutos/hhmmss para una métrica base."""
    try:
...
def phone_candidates(value: Any) -> Set[str]:
    """
    Devuelve variantes útiles para comparar DNIS/ANI.
...
def conversation_contains_trunk(
    ...
    - DNIS_PREFIX: filtra por prefijo de DNIS/Session DNIS. Para esta troncal: 2275.
    - DNIS: filtra por números exactos en TRUNK_NUMBERS, comparando ANI y DNIS.
    - EDGE: filtra por edgeId relacionado al TRUNK_NAME. Puede ser aproximado.
    - AUTO:
        1) Si hay TRUNK_DNIS_PREFIXES, usa DNIS_PREFIX.
        2) Si hay TRUNK_NUMBERS, usa DNIS exacto.
        3) Si no hay datos de troncal, trae TODO.
    """
    ...
    def match_dnis_prefix() -> Tuple[bool, str]:
        if not trunk_dnis_prefixes:
            return False, ""
        for raw in session_vals.get("dniss", set()):
            if phone_matches_prefix(raw, trunk_dnis_prefixes):
                return True, f"DNIS_PREFIX_{raw}"
        return False, ""
    ...
    # AUTO: preferir prefijo DNIS confirmado para la troncal Tigo_2.
    ok, reason = match_dnis_prefix()
    if ok:
        return True, reason
    ...
    # Usar EDGE solo como último recurso, porque puede mezclar otras troncales del mismo Edge.
...
def create_conversations_job(
    ...
    direction = clean_value(call_direction).upper() or "INBOUND"
    ...
    # Si se configuran números exactos, se prefiltran desde Genesys.
    ...
    if trunk_numbers and mode in {"AUTO", "DNIS"}:
        ...
        if number_predicates:
            ...
            log.info(
                "Job Analytics usará prefiltro DNIS/ANI exacto con %s número(s).",
                len(trunk_numbers)
            )

    if trunk_dnis_prefixes and mode in {"AUTO", "DNIS_PREFIX"}:
        log.info(
            "Filtro principal local por prefijo DNIS/Session DNIS: %s. El Job descargará voz%s y filtrará localmente.",
    ...
    for page_number, conversations, data in iter_job_result_pages(gc, job_id, page_size):
        ...
        for conv in conversations:
            ...
            row = {
                ...
                # Compatibilidad con versiones anteriores: tiempo_real = duración total de conversación.
            ...
            # Métricas adicionales de tiempo. Esto ayuda a separar duración completa vs tiempo atendido.
...
def trunk_metadata_summary(trunk_rows: List[Dict[str, Any]], trunk_name: str, trunk_id: str = "", trunk_base_id: str = "") -> Dict[str, str]:
    ...
    # Priorizar la troncal exacta por TRUNK_ID; si no está, usar coincidencias por nombre.
    selected = df.copy()
...
TIME_METRIC_SECONDS = [
    "tiempo_real_segundos",  # duración completa conversationEnd - conversationStart
...
def ensure_time_metric_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Garantiza columnas de tiempo para detalle y resúmenes."""
    out = df.copy()
...
def summarize_time_metrics(grouped: Any) -> pd.DataFrame:
    """Agrega llamadas y suma las métricas de tiempo disponibles."""
...
def export_excel(
    ...
    meta = trunk_metadata_summary(trunk_rows, trunk_name, trunk_id, trunk_base_id)

    base_cols = [
        "trunk_id", "trunk_base_id", "trunk_name", "trunk_estado", "trunk_enabled", "trunk_in_service",
        "proxy", "edges", "trunks_detectados", "modo_filtro", "call_direction", "trunk_dnis_prefixes"
    ]
    ...
    if detail_df.empty:
        ...
        # Ojo: tiempo_real = duración completa de conversación. Para operación suele convenir
        # tiempo_agente_interact o tiempo_cliente_linea, según lo que se quiera medir.
        summary_df = summarize_time_metrics(unique_detail.groupby("division", dropna=False))
        summary_df = summary_df.sort_values(["volumen_llamadas", "division"], ascending=[False, True])

        # Resumen mensual por división
        summary_month_df = summarize_time_metrics(
            unique_detail.groupby(["anio", "mes_num", "mes", "periodo", "division"], dropna=False)
        )
        summary_month_df = summary_month_df.sort_values(["anio", "mes_num", "division"], ascending=[True, True, True])
        ...
        summary_df["call_direction"] = call_direction
        summary_month_df["call_direction"] = call_direction
        summary_df["trunk_dnis_prefixes"] = ", ".join(sorted(trunk_dnis_prefixes))
        summary_month_df["trunk_dnis_prefixes"] = ", ".join(sorted(trunk_dnis_prefixes))

        metric_display_cols = []
        for sec_col in TIME_METRIC_SECONDS:
            base = sec_col[:-9] if sec_col.endswith("_segundos") else sec_col
            metric_display_cols.extend([sec_col, f"{base}_minutos", f"{base}_hhmmss"])
        extra_avg_cols = [
            c for c in ["tmo_agente_interact_minutos", "duracion_prom_cliente_linea_minutos"]
            if c in summary_df.columns
        ]

        summary_df = summary_df[
            ["division", "volumen_llamadas"] + metric_display_cols + extra_avg_cols + base_cols
        ]
        summary_month_df = summary_month_df[
            ["anio", "mes_num", "mes", "periodo", "division", "volumen_llamadas"] + metric_display_cols + extra_avg_cols + base_cols
        ]
        ...
        for sec_col in TIME_METRIC_SECONDS:
            base = sec_col[:-9] if sec_col.endswith("_segundos") else sec_col
            total_seconds_metric = int(unique_detail[sec_col].sum()) if sec_col in unique_detail.columns else 0
            total_row[sec_col] = total_seconds_metric
            total_row[f"{base}_minutos"] = round(total_seconds_metric / 60, 4)
            total_row[f"{base}_hhmmss"] = seconds_to_hhmmss(total_seconds_metric)
        total_calls = max(int(unique_detail["conversationId"].nunique()), 1)
        total_row["tmo_agente_interact_minutos"] = round((total_row.get("tiempo_agente_interact_segundos", 0) / 60) / total_calls, 4)
        total_row["duracion_prom_cliente_linea_minutos"] = round((total_row.get("tiempo_cliente_linea_segundos", 0) / 60) / total_calls, 4)
        for k, v in meta.items():
            total_row[k] = v
        total_row["modo_filtro"] = filter_mode
        total_row["call_direction"] = call_direction
        total_row["trunk_dnis_prefixes"] = ", ".join(sorted(trunk_dnis_prefixes))
        summary_df = pd.concat([summary_df, pd.DataFrame([total_row])], ignore_index=True)

    trunk_df = pd.DataFrame(trunk_rows)
    dnis_df = pd.DataFrame(
        [{"tipo": "TRUNK_DNIS_PREFIX_CONFIGURADO", "valor_normalizado": n} for n in sorted(trunk_dnis_prefixes)]
    ...
    parametros_df = pd.DataFrame([
        ...
        {"parametro": "call_direction", "valor": call_direction},
        ...
        {"parametro": "nota", "valor": "Generado usando Analytics Conversation Details Jobs. El filtro principal de esta version es DNIS/Session DNIS por prefijo 2275. EDGE se conserva solo como validacion porque puede mezclar troncales del mismo Edge."}
    ])
        ...
        diagnostico_rows.extend([
            {
                ...
                "detalle": (
                    ...
                    "Si se requiere una troncal exacta, valide que TRUNK_DNIS_PREFIXES tenga 2275 o configure TRUNK_NUMBERS con DNIS/ANI exactos. "
                    "Si desea validar volumen general por division, use FILTER_MODE=TODO."
                )
            },
            {"tipo": "Modo aplicado", "detalle": filter_mode},
            {"tipo": "TRUNK_NAME", "detalle": trunk_name if clean_value(trunk_name) else "TODAS"},
            {"tipo": "TRUNK_DNIS_PREFIXES configurados", "detalle": ", ".join(sorted(trunk_dnis_prefixes))},
            {"tipo": "TRUNK_NUMBERS configurados", "detalle": len(trunk_numbers)},
            {"tipo": "Dirección", "detalle": call_direction},
        ...
        if export_detail and not detail_exceeds_excel_limit:
            ...
            if detail_export.empty:
                detail_export = pd.DataFrame(columns=[
                    ...
                    "trunk_id", "trunk_base_id", "trunk_name", "trunk_dnis_prefixes",
                    "match_method", "edge_ids", "peer_ids", "providers", "job_id", "interval"
                ])
...
def main() -> int:
    ...
    call_direction = clean_value(args.CALL_DIRECTION).upper() or "INBOUND"
    ...
    log.info("- TRUNK_NAME: %s", trunk_name if trunk_name else "<vacío>")
    log.info("- TRUNK_ID: %s", trunk_id)
    log.info("- TRUNK_BASE_ID: %s", trunk_base_id)
    log.info("- TRUNK_DNIS_PREFIXES: %s", ", ".join(sorted(trunk_dnis_prefixes)) if trunk_dnis_prefixes else "<vacío>")
    ...
    if filter_mode == "AUTO":
        if trunk_dnis_prefixes:
            effective_mode = "DNIS_PREFIX"
        elif trunk_numbers:
            effective_mode = "DNIS"
            ...
            log.warning(
                ...
                "Se traeran llamadas de voz. Para filtrar troncal exacta, configure TRUNK_DNIS_PREFIXES o TRUNK_NUMBERS."
            )
    else:
