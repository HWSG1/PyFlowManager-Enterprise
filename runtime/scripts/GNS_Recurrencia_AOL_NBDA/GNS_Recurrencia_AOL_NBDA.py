# =========================================================
# GNS Reporte Recurrencia General AOL / NBDA
# =========================================================
#
# OBJETIVO
...
PYFLOW_PARAMS = {
    ...
    "EMAIL_SUBJECT": {
        ...
        "required": False,
        "default": "Reporte de Recurrencia AOL y NBDA"
    },
    ...
    "CALL_COST_PER_MINUTE": {
        "type": "number",
        "label": "Costo por minuto llamada recurrente USD",
        "required": False,
        "default": "0.4341"
    },
    "TRANSCRIPT_REQUEST_TIMEOUT": {
        "type": "number",
        "label": "Timeout transcripciones segundos",
        "required": False,
        "default": "10"
    },
    "TRANSCRIPT_MAX_RETRIES": {
        "type": "number",
        "label": "Reintentos por transcripcion",
        "required": False,
        "default": "1"
    },
    "TRANSCRIPT_MAX_COMMUNICATIONS_PER_CALL": {
        "type": "number",
        "label": "Maximo comunicaciones por llamada",
        "required": False,
        "default": "2"
    },
    "GRAPH_TENANT_ID": {
...
DEFAULT_CALL_COST_PER_MINUTE = 0.4341
          ...
          <tr>
            <td style="padding: 30px 40px 15px 40px;">
              ...
              <p style="margin: 0 0 24px 0; font-family: 'Neo Sans Std', 'Segoe UI', Arial, sans-serif; font-size: 15px; line-height: 1.6; color: #4b5563;">
                Comparto el detalle de <strong>Recurrencia AOL y NBDA</strong> correspondiente al periodo del <span style="color: #DA282D; font-weight: bold;">{{FECHA_INICIO}}</span> al <span style="color: #DA282D; font-weight: bold;">{{FECHA_FIN}}</span>, con corte de las {{HORA_CORTE}}.
              </p>
            </td>
          </tr>
          <tr>
...
def load_config() -> Config:
    return Config(
        ...
        call_cost_per_minute=env_float("CALL_COST_PER_MINUTE", DEFAULT_CALL_COST_PER_MINUTE),
...
def calculate_conversation_duration_seconds(start_value: Any, end_value: Any) -> float:
    """Calcula duración de la conversación usando conversationStart/conversationEnd de Genesys."""
...
def get_transcript_signed_url(
    ...
    retries = max(1, env_int("TRANSCRIPT_MAX_RETRIES", 1))
    timeout = max(5, min(config.request_timeout, env_int("TRANSCRIPT_REQUEST_TIMEOUT", 10)))
...
def fetch_recurrent_nbda_transcriptions(
    ...
    max_communications_per_call = max(1, env_int("TRANSCRIPT_MAX_COMMUNICATIONS_PER_CALL", 2))
    ...
    for processed, (_, row) in enumerate(recurrent.iterrows(), start=1):
        ...
        if not communication_ids:
            communication_ids = [""]
        communication_ids = communication_ids[:max_communications_per_call]

        text_parts: List[str] = []
        used_ids: List[str] = []
        phrases_total = 0
        for comm_index, communication_id in enumerate(communication_ids, start=1):
            if not communication_id:
                continue
            cache_key = (conversation_id, communication_id)
            try:
                logger.info(
                    "Consultando transcript NBDA %s/%s | communication %s/%s | conversationId=%s",
                    processed,
                    total_recurrent_rows,
                    comm_index,
        ...
        while True:
            ...
            for conversation in conversations:
                ...
                rows.append({
                    ...
                    "conversationEnd": conv_end,
                    "Fecha": utc_to_local_display(conv_start, config.timezone_name),
                    "Duración segundos": duration_seconds,
                    "Duración minutos": duration_seconds / 60 if duration_seconds else 0,
...
def build_client_analysis(
    ...
    call_cost_per_minute: float = DEFAULT_CALL_COST_PER_MINUTE
    ...
    for duration_col in ["Duración segundos", "Duración minutos"]:
        detail[duration_col] = pd.to_numeric(detail[duration_col], errors="coerce").fillna(0)
    ...
    def build_recurrence_table(source: pd.DataFrame, group_cols: List[str]) -> pd.DataFrame:
        output_cols = group_cols + [
            ...
            "Duración recurrente segundos",
            "Minutos recurrentes",
            "Costo por minuto USD",
            "Costo recurrente USD"
        ]
        if source.empty:
        ...
        for keys, group in per_client.groupby(group_cols, dropna=False):
            ...
            recurrent_tags = set(group.loc[group["consultas"] > 1, "externalTag"].astype(str))
            recurrent_mask = mask & source["externalTag"].astype(str).isin(recurrent_tags)
            recurrent_calls = int(source.loc[recurrent_mask, "conversationId"].nunique())
            recurrent_seconds = float(pd.to_numeric(source.loc[recurrent_mask, "Duración segundos"], errors="coerce").fillna(0).sum())
            recurrent_minutes = recurrent_seconds / 60
            recurrent_cost = recurrent_minutes * float(call_cost_per_minute or 0)
            ...
            row.update({
                ...
                "Llamadas recurrentes": recurrent_calls,
                "Duración recurrente segundos": recurrent_seconds,
                "Minutos recurrentes": recurrent_minutes,
                "Costo por minuto USD": float(call_cost_per_minute or 0),
                "Costo recurrente USD": recurrent_cost
            })
            rows.append(row)

        result = pd.DataFrame(rows, columns=output_cols)
        if not result.empty:
            result = result[pd.to_numeric(result["Clientes recurrentes"], errors="coerce").fillna(0).gt(0)]
            result = result.sort_values(["Clientes recurrentes", "Llamadas recurrentes"], ascending=[False, False])
        return result

    def build_recurrent_cost_by_conclusion(source: pd.DataFrame) -> pd.DataFrame:
        """
        Costo por conclusión usando SOLO llamadas de clientes recurrentes por canal.
        Ejemplo: si un cliente llamó 2 veces a AOL con conclusiones distintas, ambas llamadas
        se incluyen en el costo AOL porque el cliente es recurrente dentro del canal.
        """
        cols = [
            "Canal", "Conclusión", "Clientes recurrentes canal", "Llamadas recurrentes",
            "Duración recurrente segundos", "Minutos recurrentes", "Costo por minuto USD",
            "Costo recurrente USD"
        ]
        if source.empty:
            return pd.DataFrame(columns=cols)

        recurrent_parts: List[pd.DataFrame] = []
        for canal, channel_df in source.groupby("Banca", dropna=False):
            per_client = channel_df.groupby("externalTag", dropna=False)["conversationId"].nunique()
            recurrent_tags = set(per_client[per_client > 1].index.astype(str))
            if not recurrent_tags:
                continue
            tmp = channel_df[channel_df["externalTag"].astype(str).isin(recurrent_tags)].copy()
            tmp["Clientes recurrentes canal"] = len(recurrent_tags)
            recurrent_parts.append(tmp)

        if not recurrent_parts:
            return pd.DataFrame(columns=cols)

        recurrent_calls = pd.concat(recurrent_parts, ignore_index=True)
        out = (
            recurrent_calls
            .groupby(["Banca", "Nombre de conclusion"], dropna=False)
            .agg(
                **{
                    "Clientes recurrentes canal": ("Clientes recurrentes canal", "max"),
                    "Llamadas recurrentes": ("conversationId", "nunique"),
                    "Duración recurrente segundos": ("Duración segundos", "sum"),
                }
            )
            .reset_index()
            .rename(columns={"Banca": "Canal", "Nombre de conclusion": "Conclusión"})
        )
        out["Minutos recurrentes"] = pd.to_numeric(out["Duración recurrente segundos"], errors="coerce").fillna(0) / 60
        out["Costo por minuto USD"] = float(call_cost_per_minute or 0)
        out["Costo recurrente USD"] = out["Minutos recurrentes"] * float(call_cost_per_minute or 0)
        return out[cols].sort_values(["Canal", "Costo recurrente USD", "Llamadas recurrentes"], ascending=[True, False, False])

    known = detail[detail["externalTag"].astype(str).str.strip().ne("")].copy()
    # Recurrencia principal solicitada por negocio: por CANAL + CLIENTE.
    # El costo por conclusión usa únicamente llamadas de clientes recurrentes dentro de cada canal.
    recurrence = build_recurrent_cost_by_conclusion(known)
    recurrence_by_channel = build_recurrence_table(known, ["Banca"]).rename(columns={
        "Banca": "Canal"
    })
...
def create_excel(
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        # Recurrencia principal: por Canal + Cliente.
        # Esto evita perder clientes recurrentes cuando llamaron al mismo canal
        # pero por conclusiones distintas.
        ...
        # Referencia opcional: recurrencia por conclusión, no usar como volumen real del canal.
        safe_recurrence = df_recurrence if df_recurrence is not None else pd.DataFrame()
        safe_recurrence.to_excel(writer, sheet_name="Recurrencia por conclusión", index=False)
        ...
        for sheet_name in ["Resumen", "Recurrencia por canal", "Recurrencia por conclusión", "Detalle Clientes"]:
            ...
            for row in range(2, ws.max_row + 1):
                for header, col in headers.items():
                    ...
                    elif header and ("Costo" in str(header) or "USD" in str(header)):
                        ws.cell(row=row, column=col).number_format = '$#,##0.00'
                    elif header and "Minutos" in str(header):
                        ws.cell(row=row, column=col).number_format = '#,##0.00'
...
def build_client_executive_email_block(
    ...
    def channel_recurrence(canal: str) -> Tuple[int, int, float, float, float]:
        ...
        minutes = float(pd.to_numeric(rows.get("Minutos recurrentes", 0), errors="coerce").fillna(0).sum())
        cost = float(pd.to_numeric(rows.get("Costo recurrente USD", 0), errors="coerce").fillna(0).sum())
        return clients, recurrent, recurrent / clients if clients else 0.0, minutes, cost

    aol_clients, aol_recurrent, aol_recurrent_pct, aol_recurrent_minutes, aol_recurrent_cost = channel_recurrence("AOL")
    nbda_clients, nbda_recurrent, nbda_recurrent_pct, nbda_recurrent_minutes, nbda_recurrent_cost = channel_recurrence("NBDA")
    total_recurrent_cost = aol_recurrent_cost + nbda_recurrent_cost
    total_recurrent_minutes = aol_recurrent_minutes + nbda_recurrent_minutes
...
def build_client_executive_email_block(
    ...
    top_recurrence = pd.DataFrame()
    if df_recurrence is not None and not df_recurrence.empty:
        top_recurrence = df_recurrence.copy().rename(columns={
            "Conclusión": "Conclusion",
            "ConclusiÃ³n": "Conclusion",
            "Llamadas recurrentes": "Llamadas",
        })
        sort_cols = [col for col in ["Costo recurrente USD", "Llamadas"] if col in top_recurrence.columns]
        if sort_cols:
            top_recurrence = top_recurrence.sort_values(sort_cols, ascending=[False] * len(sort_cols))
        top_recurrence = top_recurrence.head(5)
    ...
    return f"""
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f8fafc;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;margin:0 0 20px 0;">
                ...
                <tr>
                  <td width="33%" style="padding:14px 12px 16px 22px;font-family:Segoe UI,Arial,sans-serif;border-top:1px solid #e2e8f0;background-color:#fff7f7;">
                    <div style="font-size:10px;color:#7f1d1d;text-transform:uppercase;font-weight:800;letter-spacing:.4px;">Costo recurrente AOL</div>
                    <div style="font-size:23px;color:#DA282D;font-weight:900;line-height:1.1;">{format_currency_usd(aol_recurrent_cost)}</div>
                    <div style="font-size:11px;color:#64748b;margin-top:3px;">{format_int(aol_recurrent_minutes)} min recurrentes</div>
                  </td>
                  <td width="33%" style="padding:14px 12px 16px 12px;font-family:Segoe UI,Arial,sans-serif;border-top:1px solid #e2e8f0;background-color:#f8fafc;">
                    <div style="font-size:10px;color:#334155;text-transform:uppercase;font-weight:800;letter-spacing:.4px;">Costo recurrente NBDA</div>
                    <div style="font-size:23px;color:#0f172a;font-weight:900;line-height:1.1;">{format_currency_usd(nbda_recurrent_cost)}</div>
                    <div style="font-size:11px;color:#64748b;margin-top:3px;">{format_int(nbda_recurrent_minutes)} min recurrentes</div>
                  </td>
                  <td width="34%" style="padding:14px 22px 16px 12px;font-family:Segoe UI,Arial,sans-serif;border-top:1px solid #e2e8f0;background-color:#111827;">
                    <div style="font-size:10px;color:#cbd5e1;text-transform:uppercase;font-weight:800;letter-spacing:.4px;">Costo recurrente total</div>
                    <div style="font-size:24px;color:#ffffff;font-weight:900;line-height:1.1;">{format_currency_usd(total_recurrent_cost)}</div>
                    <div style="font-size:11px;color:#cbd5e1;margin-top:3px;">{format_int(total_recurrent_minutes)} min recurrentes</div>
                  </td>
                </tr>
                <tr>
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reporte Genesys Recurrencia General AOL y NBDA"
    )
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("INICIO REPORTE RECURRENCIA GENERAL AOL / NBDA")
        log_params(logger, [
        ...
        if env_bool("INCLUDE_CLIENT_ANALYSIS", True):
            try:
                ...
                df_client_detail, df_recurrence, df_recurrence_by_channel, location_tables, demographic_tables = build_client_analysis(
                    df_calls,
                    df_demo,
                    call_cost_per_minute=config.call_cost_per_minute
                )
        ...
        if args.send_email and not args.dry_run:
            ...
            if not to_recipients:
                ...
                subject_base = args.subject.strip() or "Reporte de Recurrencia AOL y NBDA"
